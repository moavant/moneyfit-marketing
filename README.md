...
4. **CTA 없음** — 2026-08-19 사장님 지시로 스레드에는 다운로드 링크·"Google Play 검색" 문구를 넣지 않는다(대놓고 홍보하는 느낌 지양). 글만 꾸준히 발행하고, 유입은 프로필 링크(`mfAd/bio`)로 자연스럽게 맡긴다. 혹시 이 줄이 섞여 들어와도 `post-threads.mjs`가 게시 전 자동 제거한다.
